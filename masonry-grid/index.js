...
var MASONRY_ARR_ID = ["masonry", "masonry2"];
...
function makeMasonryLayout(masonryParentEle) {
	let masonryChilds = masonryParentEle.querySelectorAll(
		`[id^="${masonryParentEle.id}-child-"]`
	);
	...
	if (
		...
		for (let i = 0; i < masonryChilds.length; i++) {
			masonryParentEle.appendChild(
				masonryParentEle.querySelector(`#${masonryParentEle.id}-child-` + i)
			);
		}

		//remove old columns
		let masonryColumnsToRemove = document.querySelectorAll(
			`[id^="${masonryParentEle.id}-col-"]`
		);
		...
		for (let i = 0; i < masonryNumberOfColumns; i++) {
			...
			addCss2Element(newMasonryColumn, {
				...
				padding: `0 ${paddingRight + "px"} ${paddingBottom + "px"} ${
					paddingLeft + "px"
				}`,
			});
	...
	let masonryColumnsCurrent = masonryParentEle.querySelectorAll(
		`[id^="${masonryParentEle.id}-col-"]`
	);
	...
	let masonryColumnsCurrentHeightKeys = Object.keys(
		masonryColumnsCurrentHeight
	);
	let masonryColumnMinHeightName = "";

	for (let i = 0; i < masonryChilds.length; i++) {
		let masonryChildEle = masonryParentEle.querySelector(
			`#${masonryParentEle.id}-child-` + i
		);
		...
		for (let j = masonryColumnsCurrentHeightKeys.length - 1; j >= 0; j--) {
			if (
				masonryColumnsCurrentHeight[masonryColumnsCurrentHeightKeys[j]] <=
				masonryColumnMinHeightValue
			) {
		...
		masonryColumnsCurrentHeight[masonryColumnMinHeightName] +=
			masonryChildHeight;
		...
		masonryParentEle
			.querySelector("#" + masonryColumnMinHeightName)
			.appendChild(masonryChildEle);
	}

	for (let i = 0; i < masonryColumnsCurrent.length; i++) {
		masonryColumnsCurrentHeight[masonryColumnsCurrent[i].id] =
			masonryColumnsCurrent[i].clientHeight;
	}
	addCss2Element(masonryParentEle.parentElement, {
		height:
			Math.max(
				...Object.keys(masonryColumnsCurrentHeight).map(
					(key) => masonryColumnsCurrentHeight[key]
				)
			) + "px",
	});
}
